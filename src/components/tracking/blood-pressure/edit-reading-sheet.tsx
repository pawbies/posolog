...
export default function EditReadingSheet({ isPresented, reading, onDismiss }: Props) {
  ...
  // Seed the fields from the reading each time the sheet opens. Deliberately keyed
  // on the id only: a live-query refresh that hands us a new object for the same
  // row must not overwrite whatever is half-typed in the inputs.
  useEffect(() => {
    if (!isPresented || !reading) return;
    setValues({
      systolic: String(reading.systolic),
      diastolic: String(reading.diastolic),
      pulse: reading.pulse == null ? "" : String(reading.pulse),
    });
    setReadingAt(reading.readingAt);
    setStep(null);
    setError(null);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isPresented, reading?.id]);
  ...
  const save = async () => {
    ...
    try {
      await db
        ...
        .set({ systolic, diastolic, pulse, readingAt })
  ...
  const canSave = reading !== null && systolic !== null && diastolic !== null && !saving;
